...
public class CourseTest {
    ...
    public static final ResourceTestRule resources = ResourceTestRule.builder()
            .addResource(new CourseResource(new CourseService(dao)))
            .build();

    private final Course expectedCourse = new Course("IIAD", "Inleiding Algoritmen en Datastructuren",
            null, null);
    ...
    public void insertCourse() {
        Response response = resources.target("/courses").request().post(Entity.json(expectedCourse));
        assertEquals(response.getStatus(), 204);
    }

    @Test
    public void updateCourse() {
        Response response = resources.target("/courses/IIAD").request().put(Entity.json(expectedCourse));
        assertEquals(response.getStatus(), 204);
    }
