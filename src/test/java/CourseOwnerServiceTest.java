...
public class CourseOwnerServiceTest {
    ...
    private final CourseOwner expectedCourseOwner =
        new CourseOwner("modulelieder@ipsenh.nl", "IAD1");
    ...
    @Before public void setup() {
        when(COURSE_OWNER_DAO.getCourseOwnerByEmail(eq("modulelieder@ipsenh.nl")))
            .thenReturn(expectedCourseOwner);
    }

    @Test public void testGetCourseOwner() {
        CourseOwner courseOwner = resources.target("/courseowners/modulelieder@ipsenh.nl").request()
            .get(CourseOwner.class);
        assertTrue(expectedCourseOwner.equals(courseOwner));
        verify(COURSE_OWNER_DAO).getCourseOwnerByEmail("modulelieder@ipsenh.nl");
    }

    @Test public void testInsertCourseOwner() {
        Response response = resources.target("/courseowners/").request().post(Entity.json(expectedCourseOwner));
