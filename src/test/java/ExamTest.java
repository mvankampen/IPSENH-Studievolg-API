...
public class ExamTest {
    ...
    @ClassRule public static final ResourceTestRule resources =
        ResourceTestRule.builder().addResource(new ExamResource(new ExamService(dao))).build();
    ...
    @Test public void getAllExams() {
        Collection<Exam> testExams =
            resources.target("/exams").request().get(new GenericType<List<Exam>>() {
            });
        assertEquals(expectedExams.size(), testExams.size());
        verify(dao).getAll();
    }

    @Test public void getExamsByCourse() {
        Collection<Exam> testExams =
            resources.target("/exams/IPSENH").request().get(new GenericType<List<Exam>>() {
            });
        assertEquals(expectedExams.size(), testExams.size());
    ...
    @Test public void updateExam() {
        Response response =
            resources.target("/exams/ToetsA").request().put(Entity.json(expectedExam));
        assertEquals(response.getStatus(), 204);
    }
