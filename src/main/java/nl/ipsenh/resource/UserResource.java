 ...
 */
@Path("/users") @Produces(MediaType.APPLICATION_JSON) @Consumes(MediaType.APPLICATION_JSON)
public class UserResource {
    ...
    @GET @JsonView(View.Public.class) @RolesAllowed("admin") @Timed
    public Collection<User> getAllUsers() {
        return service.getAllUsers();
    }

    @GET @JsonView(View.Public.class) @RolesAllowed("admin") @Path("/{email}") @Timed
    public User getUserById(@PathParam("email") String userEmail) {
        return service.getUserByEmail(userEmail);
    }

    @POST @JsonView(View.Public.class) @RolesAllowed("admin") public void insertUser(User user) {
        service.insertUser(user);
    }

    @PUT @JsonView(View.Protected.class) @Path("/{email}") @RolesAllowed("admin")
    public void updateUser(@PathParam("email") String userEmail, User user) {
        service.updateUser(userEmail, user);
    }

    @PUT @JsonView(View.Protected.class) @Path("update/{email}")
    public void updatePassword(@PathParam("email") String email, User user) {
        service.updatePassword(email, user);
    }

    @GET @Path("/me") @JsonView(View.Private.class)
